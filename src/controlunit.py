...
from datapath import DataPath, MuxTdSel, MuxSSel, MuxAluLeftSel, MuxAluRightSel, MuxDataReadSel, MuxSrSel, AluOp
...
class MicroInstruction:
  def __init__(
    ...
    alu_op: AluOp = AluOp.PASSTHROUGH_L,
    ...
    write_io: bool = False,
    next_mpc_idx: Optional[int] = None
  ):
    ...
    self.next_mpc_idx: Optional[int] = next_mpc_idx

class ControlUnit:
  def __init__(self, mem_size: int, mprogram: List[MicroInstruction], state_decoder_map: Dict[Tuple[Opcode, int], int], data_path: DataPath):
  ...
  def tick(self) -> None:
    ...
    if self.mr.latch_td:
      self.data_path.latch_td(self.mr.select_td, self.mr.select_data_read, self.mr.alu_op, self._get_prefetch_value())

    if self.mr.latch_s:
      self.data_path.latch_s(self.mr.select_s)

    if self.mr.latch_sr:
      self.data_path.latch_sr(self.mr.select_sr, self.mr.alu_op, self.mr.alu_l_sel, self.mr.alu_r_sel)
    ...
    if self.mr.latch_pc:
      match self.mr.select_pc:
        ...
        case MuxPcSel.TR:
          next_pc = self.tr
        case _:
          raise Exception("Unknown state for MuxPC")

    next_tr: int = self.tr
    if self.mr.latch_tr:
      if self.mr.select_tr == MuxTrSel.PC:
        next_tr = self.pc
      elif self.mr.select_tr == MuxTrSel.STACK_PREV:
    ...
    if self.mr.latch_mpc:
      if self.mr.select_mpc == MuxMpcSel.MPC_PLUS_1:
        next_mpc = self.mpc + 1
      elif self.mr.select_mpc == MuxMpcSel.STATE_DECODER:
        if self.mr.next_mpc_idx is not None:
          next_mpc = self.mr.next_mpc_idx
        else:
